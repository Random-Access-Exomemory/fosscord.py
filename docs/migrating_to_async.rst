...
See :ref:`fosscord-api-events` for more information. Other removed events include ``on_socket_closed``, ``on_socket_receive``, and ``on_socket_opened``.
...
.. code-block:: python3
    ...
    server.region == fosscord.ServerRegion.us_west
    member.status = fosscord.Status.online
    channel.type == fosscord.ChannelType.text

The main reason for this change was to reduce the use of finicky strings in the API as this
could give users a false sense of power. More information can be found in the :ref:`fosscord-api-enums` page.
...
In earlier versions of fosscord.py, ``client.run()`` was a blocking call to the main thread
